...
//   실행하면 파이프라인 _resolve_backend 가 azure→bedrock 으로 폴백하고 Bedrock 은 IAM 명시
//   거부(QA-DenyBedrockInvoke-20260810)라 **평가가 통째로 실패**한다. 실패 사유도 화면에는
//   "AccessDenied" 로만 보여 원인 추적이 어렵다. 그래서 고르기 전에 가용성을 물어본다.
//
...
app.get('/api/llm/backends', async (req, res) => {
    try {
...
app.get('/api/admin/kms-items', requireAdmin, async (req, res) => {
    try {
        const orgId = resolveActiveOrgId(req);
        if (!orgId) {
...
app.put('/api/admin/kms-items', requireAdmin, async (req, res) => {
...
app.post('/api/admin/kms-index', requireAdmin, async (req, res) => {
    const orgId = resolveActiveOrgId(req, { strict: true });
    if (!orgId) {
