        ...
        <div className="bg-white rounded-xl border border-[var(--border)] shadow-[0_1px_2px_rgba(16,24,40,0.04)] overflow-hidden">
            <div className="px-5 py-4 border-b border-[var(--muted)] flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[14px] font-semibold text-[var(--ink-900)]">코칭 액션 (AI 자동 추천)</h3>
                </div>
                {data.teamAvg > 0 && (
                    <span className="text-[12px] text-[var(--ink-500)] tabular-nums">팀 평균 {data.teamAvg}점</span>
                )}
            </div>

            {data.priority.length === 0 && !data.best ? (
                <div className="px-5 py-10 text-center text-[var(--ink-500)] text-[14px]">
                ...
                <div className="divide-y divide-[var(--muted)]">
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1.5">
                                        ...
                                        <span className="text-[14px] font-semibold text-[var(--ink-900)]">{a.name}</span>
                                        {a.role && <span className="text-[12px] text-[var(--ink-500)]">· {a.role}</span>}
                                    </div>
                                    <div className="text-[13px] text-[var(--ink-700)]">
                                        종합 <strong className="tabular-nums">{a.avg}점</strong>
                                        <span className="text-[var(--warning)] ml-1.5 tabular-nums">
                                            (팀 평균 {a.diff >= 0 ? '+' : ''}{a.diff})
                                        </span>
                                        <span className="text-[var(--ink-500)] ml-1.5 tabular-nums">· 콜 {a.count}건</span>
                                    </div>
                                    {a.weakest && (
                                        <div className="text-[12px] text-[var(--ink-500)] mt-0.5">
                                            보완 필요 항목: <strong className="text-[var(--ink-700)]">{a.weakest}</strong>
                                        </div>
                                    )}
                                </div>
                                <span className="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full bg-[var(--warning-soft)] text-[var(--warning)] text-[11px] font-semibold">
                                    우선 코칭
                                </span>
                            </div>
                        </div>
                    ...
                    {data.best && (
                        <div className="px-5 py-4">
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1.5">
                                        <Award size={14} className="text-[var(--success)]" />
                                        <span className="text-[14px] font-semibold text-[var(--ink-900)]">{data.best.name}</span>
                                        {data.best.role && (
                                            <span className="text-[12px] text-[var(--ink-500)]">· {data.best.role}</span>
                                        )}
                                    </div>
                                    <div className="text-[13px] text-[var(--ink-700)]">
                                        종합 <strong className="tabular-nums">{data.best.avg}점</strong>
                                        <span className="text-[var(--success)] ml-1.5 tabular-nums">
                                            (팀 평균 +{data.best.diff})
                                        </span>
                                        <span className="text-[var(--ink-500)] ml-1.5 tabular-nums">· 콜 {data.best.count}건</span>
                                    </div>
                                    <div className="text-[12px] text-[var(--ink-500)] mt-0.5">
                                        추천: 베스트 콜 → 신입 교육 콘텐츠
                                    </div>
                                </div>
                                <span className="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full bg-[var(--success-soft)] text-[var(--success)] text-[11px] font-semibold">
                                    베스트 콜 후보
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="px-5 py-3 bg-[var(--background-soft)] text-[12px] text-[var(--ink-500)] flex items-center justify-between">
                        <span>
                            전체 코칭 대상 <strong className="text-[var(--ink-900)] tabular-nums">{data.totalCandidates}명</strong>
                        </span>
                        <span className="text-[var(--ink-500)]">자동 리포트 → 익일 09:00 매니저 메일 발송 예약</span>
                    </div>
                </div>
            )}
        </div>
