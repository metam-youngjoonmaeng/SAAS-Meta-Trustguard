...
-- ============================================================
-- ★ 2026-07-27 — ON CONFLICT 를 DO UPDATE → DO NOTHING 으로 전환.
--   원래 의도는 위 주석대로 '한 번 정렬'이었으나, seed-if-empty.sh 가 매 기동 이 파일을 재적용하므로
--   DO UPDATE 는 그 의도를 넘어 '매 기동 강제 되돌림'으로 동작했다. super_admin 이 업종 기본
--   평가항목·펜타곤 축을 수정·비활성화해도 컨테이너 재기동 한 번에 2026-07-06 스냅샷 값으로
--   되돌아갔고(81건), 되돌림이 조용해서 편집이 사라진 줄도 몰랐다.
--   DO NOTHING 이면 빈 볼륨 첫 부팅의 시딩 목적(행 부재 → INSERT)은 그대로이고, 이미 존재하는
--   행은 운영 데이터로 존중된다. 스냅샷을 다시 강제로 맞춰야 할 때는 이 파일을 수동 실행하지 말고
--   의도를 명시한 별도 마이그레이션(64+)을 추가한다.
-- ============================================================
...
[출력] {"rating":"우수|보통|주의|실패","analysis":"핵심만 간결하게 2-3문장"}', 't') ON CONFLICT (domain_id, axis_no) DO NOTHING;
...
- 0점: 소속/실명/본인확인 질의 중 2개 이상 누락, 또는 본인확인 없이 본론 진입', '응대·표현', 'numeric', 6, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 종료 인사 미진행', '응대·표현', 'numeric', 3, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 불편·거절 상황에 사과·쿠션어·호응 전무, ''네네'' 단순 반복함', '응대·표현', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 반말·명령·짜증·다그침·비아냥 사용, 반말·욕설성 어휘 시 즉시 해당', '응대·표현', 'numeric', 5, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 동문서답·반복 재질문, 또는 고객 2회 이상 재설명 유발함', '니즈파악·경청', 'numeric', 6, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 이해 불가 수준의 설명으로 상담 포기를 유발함', '설명·전달력', 'numeric', 6, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 오안내(정정 필요)·규정 위반 안내·금지멘트 사용 중 하나 이상 발생. ※ 금지멘트 또는 사실과 다른 요율/한도/조건 안내 시 즉시 0점', '정확성·해결력', 'numeric', 13, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 회피·떠넘김 등 해결 의지 없음', '정확성·해결력', 'numeric', 5, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 본인확인 미이행 후 계좌/거래 정보 안내, 확인 항목 2개 이상 누락, 또는 생년월일 미확인·전산 불일치', '컴플라이언스', 'numeric', 12, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 정보 선언급·동의 없는 활용·제3자 유출 중 하나 이상 발생', '컴플라이언스', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 핵심 필수안내(금리·연체 불이익·수수료·지급정지 절차 등) 미이행, 잘못된 안내 시 오안내와 중복 0점', '컴플라이언스', 'numeric', 12, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 적합성 미확인 후 권유 또는 원금비보장·중도해지 손실 등 핵심 고지 누락함', '컴플라이언스', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 사고 정황을 놓치거나 지급정지/정지 등 즉시 조치 미안내함', '컴플라이언스', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 인사말·소속·실명 중 다수 요소 누락 또는 첫 인사 자체 부재', '응대·표현', 'numeric', 5, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 추가 문의 확인·종료 인사·실명 모두 누락함', '응대·표현', 'numeric', 5, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 고객의 선행 인사·감사가 있었음에도 화답이 누락됨', '응대·표현', 'numeric', 3, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 사과·쿠션어·호응 표현 전반 부재 또는 단순 감정 반복으로 상황 맥락 공감 미충족', '응대·표현', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 반말·명령형·훈계조·혼잣말 등 부적절 화법 다수 사용 또는 확신 없는 표현으로 고객 불안 유발', '응대·표현', 'numeric', 7, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 용건 미확인, 상품 특정 누락, 재복창 부재, 또는 이미 언급/조회 가능 내용 반복 재질문', '니즈파악·경청', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 내부 용어·약어 다수 사용 또는 두괄식 미적용·과도한 장황으로 고객 이해 곤란, 반복 되물음 유발', '설명·전달력', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 정책 불부합 오안내, 복합 문의 다수 질문 누락, 또는 금지·오인 멘트 사용 등 미충족·위반함', '정확성·해결력', 'numeric', 18, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 책임 회피·떠넘김으로 일관하거나 대안·후속조치·셀프서비스 안내가 전혀 없음', '정확성·해결력', 'numeric', 10, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 필수안내 항목을 전달하지 않거나 대부분 누락함', '설명·전달력', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 본인확인 미수행 또는 민감 정보 안내가 본인확인보다 선행되어 순서 위반함', '컴플라이언스', 'numeric', 8, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 규정상 불가 정보를 제공(정보 유출)하거나 제3자에게 안내 범위 위반, 또는 안내 가능 업무 회피', '컴플라이언스', 'numeric', 6, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
...
- 0점: 전산 정보를 고객 확인 전 선언급하거나, 동의 없이 정보를 활용하거나 정보 취급 가이드를 위반함', '컴플라이언스', 'numeric', 6, 't') ON CONFLICT (domain_id, order_no) DO NOTHING;
