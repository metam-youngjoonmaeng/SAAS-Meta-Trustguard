...
SELECT 'R1_calls_agg', c."ID", c."AI_SCORE", c."TOTAL_SCORE", c.review_status,
       (SELECT COALESCE(SUM(CASE WHEN ch.validation_time LIKE '배점%'
              ...
              ELSE 0 END), 0)
        FROM qa_checklist_rows ch WHERE ch."ID" = c."ID") AS max_total
FROM qa_calls c
ORDER BY c."ID";
...
SELECT 'R3_checklist', "ID", order_no, category, item, agent_utterance, validation_time
...
FROM qa_ksqi_rows ORDER BY "ID", item_number;
SELECT 'R7_ksqi_evidence', "ID", item_number, seq, speaker, quote
FROM qa_ksqi_evidence ORDER BY "ID", item_number, seq;
...
FROM qa_ksqi_summary ORDER BY "ID";
...
UNION ALL SELECT 'R16_counts','qa_conversations', COUNT(*) FROM qa_conversations
UNION ALL SELECT 'R16_counts','qa_evaluation_rows', COUNT(*) FROM qa_evaluation_rows
UNION ALL SELECT 'R16_counts','qa_checklist_rows', COUNT(*) FROM qa_checklist_rows
UNION ALL SELECT 'R16_counts','qa_analysis_report', COUNT(*) FROM qa_analysis_report
UNION ALL SELECT 'R16_counts','qa_ksqi_rows', COUNT(*) FROM qa_ksqi_rows
UNION ALL SELECT 'R16_counts','qa_ksqi_evidence', COUNT(*) FROM qa_ksqi_evidence
UNION ALL SELECT 'R16_counts','qa_ksqi_summary', COUNT(*) FROM qa_ksqi_summary
