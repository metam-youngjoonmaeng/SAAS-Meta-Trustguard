...
-- ------------------------------------------------------------
-- domain_default_eval_items(47 테이블) 콘텐츠. 신규 브랜드 생성/도메인 변경 시
...
VALUES
    (1, 1, '응대·표현', '첫인사 · 본인확인 도입', '인사말+소속+상담사 실명 + 고객 본인 여부 확인 도입(‘○○○ 고객님 본인 맞으시죠?’) + 고객 답변 수령.', '점수 단계: 6 / 3 / 0
...
- 거절: 아니요 / 안 할래요 / 필요 없어요 / 됐습니다 / 그만 하세요 / 안듣고 싶어요 → 가입/진행 불가 — 중단 여부 확인', NULL, 'numeric', 6, true),
    (1, 2, '응대·표현', '끝인사', '종료 인사 + 소속/상담사 실명 전달.', '점수 단계: 3 / 1 / 0
...
## STT 주의·보정
멘트 변형 허용(의미 매칭).', NULL, 'numeric', 3, true),
    (1, 3, '응대·표현', '공감/호응 · 쿠션어', '고객 불편/부정 상황 즉각 사과, 거절·양해 시 쿠션어, 상황에 맞는 호응 1회↑. 단순 감정 위로가 아닌 상황 공감.', '점수 단계: 8 / 4 / 0
...
## STT 주의·보정
표현 존재·적절성까지만 평가(음성 톤 제외). 대기/묵음은 텍스트 불가로 제외.', NULL, 'numeric', 8, true),
    (1, 4, '응대·표현', '정중한 언어표현', '정중·전문 화법. 반말·명령·훈계·혼잣말·습관어/사족어 미사용. (금지 예: ‘방금 말씀드렸잖아요’, ‘그럼 어떻게 해드려요?’)', '점수 단계: 5 / 3 / 0
...
- [욕설·비하] (욕설·비속어·인격 비하 표현) → 불친절 패널티(-20/콜 0점)', NULL, 'numeric', 5, true),
    (1, 5, '정확성·설명', '니즈파악 · 재복창', '용건 확인 + 핵심 요지 요약·재복창. 거래/계좌/상품 특정. 불필요한 재질문 없음.', '점수 단계: 6 / 3 / 0
...
## STT 주의·보정
계좌·상품·증권 식별자 STT 오인식 주의. 재질의 카운트는 화자 라벨 의존.', NULL, 'numeric', 6, true),
    (1, 6, '정확성·설명', '설명력 · 전달력', '고객 눈높이의 쉬운 설명(전문용어 지양), 두괄식 핵심 전달, 이해 확인하며 진행.', '점수 단계: 6 / 3 / 0
...
- "거치식" → 고객용 표현: "이자만 받다가 만기에 원금(풀어 설명)" (풀어 안내)', NULL, 'numeric', 6, true),
    (1, 7, '정확성·설명', '오안내 · 정확한 안내 · 금지멘트', '계좌·이체·대출·카드·상품 전반 정확 안내, 추측성 단정 회피, 금지멘트 미사용(펀드 원금보장·대출 최저금리·피싱 전액환급 단정 등).', '점수 단계: 13 / 7 / 0
...
- [규정 통보] 금지: "원래 그래요" (변형 예: 규정이 그래요 / 원칙상 안돼요) / 사유: 근거 없는 통보·설득 실패 / 권장 대체: 규정 근거를 들어 설명', NULL, 'numeric', 13, true),
    (1, 8, '정확성·설명', '적극성 · 해결의지', '책임 있는 처리, 불가 시 대안·후속조치 제시, 추가 불편 확인. 성급한 종결 유도 금지.', '점수 단계: 5 / 3 / 0
...
도움드리지 못해 죄송합니다. → 적극성 → 가점 신호', NULL, 'numeric', 5, true),
    (1, 9, '컴플라이언스(금융규정)', '본인확인 절차 · 순서 · 항목', '계좌/거래/정보 안내·처리 이전 본인확인 선행. 2가지 정보(성함+생년월일 등) 또는 본인인증 완료, 통화자명·명의자·관계 확인(제3자 시). 본인확인 → 안내/처리 순서.', '점수 단계: 12 / 6 / 0
...
신청할게요 / 그렇게 해주세요 (할게요) → 동의·확인 성립으로 인정
- 불인정·불분명: 글쎄요 / 생각해볼게요 / 잘 모르겠어요 / (무응답) → 미수득 → 0점·신뢰도 강등
- 거절: 아니요 / 안 할래요 / 필요 없어요 / 됐습니다 / 그만 하세요 / 안듣고 싶어요 → 가입/진행 불가 — 중단 여부 확인', NULL, 'numeric', 12, true),
    (1, 10, '컴플라이언스(금융규정)', '개인정보 · 정보보호', '고객 정보 선언급 금지, 동의 없는 활용 금지, 제3자 정보 미제공, 정보 취급 가이드 준수.', '점수 단계: 8 / 4 / 0
...
## STT 주의·보정
선언급 판정은 발화 순서에 의존(텍스트로 가능). 정보 토큰 STT 오인식 주의.', NULL, 'numeric', 8, true),
    (1, 11, '컴플라이언스(금융규정)', '업무별 필수안내 (이체·대출·계좌·카드 등)', '문의 업무유형별 필수안내 이행 — 이체(한도·수수료·반영시점), 대출(금리유형·상환·중도상환수수료·연체 불이익), 계좌(서류·해지 영향), 카드(연회비·실적·분실 정지·재발급), 외환(서류·한도·소요).', '점수 단계: 12 / 6 / 0
...
- [10.2 기타/일반>기타][권장] 문의에 맞는 적절 부서/채널을 안내.
  ...
  · 미충족 예: "그건 저희가 안 해요. (적절 채널 안내 없이 회피)"', NULL, 'numeric', 12, true),
    (1, 12, '컴플라이언스(금융규정)', '금융상품 가입 적합성 · 필수고지', '예적금·펀드 등 가입 시 적합성(투자성향) 확인 + 필수 고지(예적금 만기·중도해지 손실 / 펀드 원금비보장·위험등급·수수료). (가입성 상담에 한함)', '점수 단계: 8 / 4 / 0
...
신청할게요 / 그렇게 해주세요 (할게요) → 동의·확인 성립으로 인정
- 불인정·불분명: 글쎄요 / 생각해볼게요 / 잘 모르겠어요 / (무응답) → 미수득 → 0점·신뢰도 강등
- 거절: 아니요 / 안 할래요 / 필요 없어요 / 됐습니다 / 그만 하세요 / 안듣고 싶어요 → 가입/진행 불가 — 중단 여부 확인', NULL, 'numeric', 8, true),
    (1, 13, '컴플라이언스(금융규정)', '보안사고 대응 (분실·피싱·부정거래)', '분실·도난·보이스피싱·부정거래 인지 시 즉시 정지/지급정지·신고·피해구제 절차 안내 등 적절 대응. 사고 정황 누락 금지.', '점수 단계: 8 / 4 / 0
...
### 정멘트 골격 (피싱 지급정지)
- [BK-S04 보이스피싱 지급정지 안내] 지급정지 신청·피해구제 절차 안내 / 필수 요소·순서: 지급정지 + 피해구제 / 허용 변형: 멘트 변형 허용', NULL, 'numeric', 8, true),
    (3, 1, '응대·표현', '첫인사', '인사말 + 소속(센터/부서) + 상담사 실명 3요소를 모두 발화. 본 문의 진입 전 완료. 호전환 시 ‘연결받았습니다’+실명 인정(소속 생략 허용).', '점수 단계: 5 / 3 / 0
- 5점: 3요소 모두 명확.
...
## STT 주의·보정
실명 STT 오인식 잦음 → 끝맺음 패턴으로 추출, 단순 미일치로 0점 금지. 화자 라벨이 상담사 첫 턴을 맞게 가리키는지 확인.', NULL, 'numeric', 5, true),
    (3, 2, '응대·표현', '끝인사 · 추가문의 확인', '종료 전 추가 문의 확인(‘더 도와드릴 점 없으십니까’) + 종료 인사 + 상담사 실명. 추가 문의 시 응대 후 끝인사 재이행.', '점수 단계: 5 / 3 / 0
...
## STT 주의·보정
‘선종료(상담사 먼저 끊음)’는 통화 로그 영역이라 평가 안 함 — 발화 순서로 판정 가능한 ‘추가문의 확인·끝인사 누락’만 본다.', NULL, 'numeric', 5, true),
    (3, 3, '응대·표현', '화답인사', '고객이 먼저 인사/감사 표현 시 상담사가 적절히 화답(‘네, 안녕하세요’, ‘감사합니다’). 고객 인사 없으면 만점 자동.', '점수 단계: 3 / 1 / 0
...
## STT 주의·보정
고객 ‘네/안녕하세요’ 단답이 STT에서 누락되지 않도록 보존. 화자 라벨 뒤바뀜 시 오판 주의.', NULL, 'numeric', 3, true),
    (3, 4, '응대·표현', '공감/호응 · 쿠션어', '① 불편/부정 상황 즉각·구체적 사과, ② 거절·양해 시 쿠션어(‘번거로우시겠지만~’), ③ 상황에 맞는 호응 1회↑. 단순 감정이 아닌 상황 맥락 공감.', '점수 단계: 8 / 4 / 0
...
### 정멘트 (규정 멘트 · 필수 요소·순서)
- 고객 불만 상황: “불편을 드려 죄송합니다.” — 필수 요소: 사과 + 공감', NULL, 'numeric', 8, true),
    (3, 5, '응대·표현', '정중한 언어표현', '정중·전문 화법. 반말·명령형·지시형·훈계조·혼잣말·과도한 사족어 미사용. 확신 없는 표현으로 불안 유발하지 않음.', '점수 단계: 7 / 4 / 0
- 7점: 전 구간 정중·전문.
...
- 혼잣말·사족어: 음.. 그게.. / 아 진짜 / (잦은) 어~ → 정중한 언어표현 → 빈도 감점
- 불확신: 글쎄요 / 아마 그럴걸요 / 잘 모르겠는데 → 설명력·정중 → 감점(불안 유발)
- 욕설·비하: (욕설·비속어·인격 비하 표현) → 불친절 패널티(-20/콜 0점)', NULL, 'numeric', 7, true),
    (3, 6, '문제 해결', '니즈파악 · 상품 특정 · 재복창', '① 용건 확인, ② 문의 상품을 ‘주문일+상품명’ 또는 ‘브랜드+상품명’으로 특정, ③ 핵심 요지 재복창. 이미 언급/조회 가능 내용 재질문 금지.', '점수 단계: 8 / 4 / 0
...
## STT 주의·보정
상품명·주문일 STT 오인식 시 특정 오판 → 주문 데이터 대조 권장. 재질의 카운트는 화자 라벨 의존.', NULL, 'numeric', 8, true),
    (3, 7, '문제 해결', '설명력 (내부용어 지양·두괄식)', '고객 눈높이의 쉬운 설명, 사내 용어/약어 미사용(‘구확취’→‘구매확정 후 취소’), 두괄식 핵심 우선, 장황하지 않음.', '점수 단계: 8 / 4 / 0
- 8점: 쉬운 용어·두괄식·간결.
...
- 미출 → 아직 출고전 상태', NULL, 'numeric', 8, true),
    (3, 8, '문제 해결', '정확한 안내 · 오안내 · 복합문의 답변', '① 정책/규정에 부합하는 정확한 안내, ② 추측성·단정 회피, ③ 복합 문의 시 모든 질문 누락 없이 답변, ④ 금지/오인 멘트 미사용.', '점수 단계: 18 / 9 / 0
...
- [규정 통보] 금지: 원래 그래요 (변형 예: 규정이 그래요) — 사유: 근거 없는 통보·설득 실패 → 권장 대체: 규정 근거를 들어 설명', NULL, 'numeric', 18, true),
    (3, 9, '문제 해결', '적극성 · 대안 · 셀프서비스 안내', '처리 가능 건은 책임지고 완료, 불가 시 대안/후속조치 제시, 자가 해결을 위한 어드민 경로·매뉴얼 안내, 추가 불편 선제 확인.', '점수 단계: 10 / 5 / 0
...
- 회피·소극: 저희 부서가 아니라 모른다 / 원래 그래요 / 어쩔 수 없어요 → 적극성 → 감점/0점
- 떠넘김: 앱에서 직접 하시면 돼요(처리 안 함) / 홈페이지 보세요 → 적극성 → 부분 이하
- 능동·해결: 제가 처리해 드리겠습니다 / 바로 도와드리겠습니다 / 확인해서 연락드리겠습니다 → 적극성 → 가점 신호', NULL, 'numeric', 10, true),
    (3, 10, '문제 해결', '필수안내 (반품·환불 절차/소요일)', '문의 유형별 필수안내 누락 없이 전달: 반품/교환 절차·조건, 환불 금액·소요일, 회수 방법, 약속 시간 등.', '점수 단계: 8 / 4 / 0
- 8점: 해당 유형 필수안내 모두 이행.
- 4점: 부가 항목 누락(핵심 이행).
...
#### 기타/일반 > 기타 (8.2)
- [권장] 문의에 맞는 적절 채널을 안내. | ✓ 그 부분은 담당 팀으로 연결해 드릴게요. | ✗ 그건 저희 일 아니에요. (적절 채널 안내 없이 회피)', NULL, 'numeric', 8, true),
    (3, 11, '컴플라이언스', '본인확인 절차 · 순서', '민감 정보 안내 이전 본인확인 선행. 안내에 필요한 식별 정보(주문번호/연락처/성함) 적정 확인. 본인확인 → 안내 발화 순서 준수.', '점수 단계: 8 / 4 / 0
- 8점: 본인확인 선행 + 적정 항목 확인.
...
※ 실제 고객정보(PII)는 미포함 — 전산 일치 검증에만 연계. 절차·순서·항목은 전사 텍스트로 평가 가능.', NULL, 'numeric', 8, true),
    (3, 12, '컴플라이언스', '안내 범위 준수', '요청자(본인/제3자)별 안내 가능 범위 준수. 본인이라도 규정상 불가 정보는 미제공+사유 안내. 안내 가능 업무 회피 금지.', '점수 단계: 6 / 3 / 0
...
- 본인인증 / 회원 — 정답 기준: 본인 확인 절차 안내 필수 / 대표 오안내: 본인확인 절차 누락 안내 / 근거: [회원정책]
- 배송·출고일 / 배송 — 정답 기준: 주문제작상품 or  예약배송상품 등 배송예정일 확인 후 안내 / 대표 오안내: 실제 출고 예정일과 다르게 임의 안내 / 근거: [배송정책]
- 판매중지·품절상품/ 배송 — 정답 기준: 판매중지·품절상품 배송 가능여부 확인 후 안내 / 대표 오안내: 재입고 확정 오안내 / 근거: [배송정책]', NULL, 'numeric', 6, true),
    (3, 13, '컴플라이언스', '개인정보 취급 · 선언급 금지', '고객 정보 선언급 금지(전산 확인 내용을 고객 확인 전 먼저 말하지 않음), 동의 없는 활용 금지, 정보 취급 가이드 준수.', '점수 단계: 6 / 3 / 0
- 6점: 가이드 준수.
...
## STT 주의·보정
선언급 판정은 발화 순서에 의존(텍스트로 가능). 정보 토큰 STT 오인식 주의.', NULL, 'numeric', 6, true)
